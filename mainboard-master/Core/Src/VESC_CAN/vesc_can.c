...
void comm_can_set_pos_pid(uint8_t controller_id, float pos_kp, float pos_ki, float pos_kd) {
	int32_t send_index = 0;
	uint8_t buffer[12];
	buffer_append_int32(buffer, (int32_t)(pos_kp * 1e3), &send_index);
	buffer_append_int32(buffer, (int32_t)(pos_ki * 1e3), &send_index);
	buffer_append_int32(buffer, (int32_t)(pos_kd * 1e5), &send_index);

#if defined VESC_CAN1
	CAN_TxMsgEID(&hcan1, controller_id | ((uint32_t)CAN_PACKET_SET_POS_KP << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan1, controller_id | ((uint32_t)CAN_PACKET_SET_POS_KI << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan1, controller_id | ((uint32_t)CAN_PACKET_SET_POS_KD << 8), buffer, send_index / 3);
#elif defined VESC_CAN2
	CAN_TxMsgEID(&hcan2, controller_id | ((uint32_t)CAN_PACKET_SET_POS_KP << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan2, controller_id | ((uint32_t)CAN_PACKET_SET_POS_KI << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan2, controller_id | ((uint32_t)CAN_PACKET_SET_POS_KD << 8), buffer, send_index / 3);
#endif
}

void comm_can_set_rpm_pid(uint8_t controller_id, float rpm_kp, float rpm_ki, float rpm_kd) {
	int32_t send_index = 0;
	uint8_t buffer[12];
	buffer_append_int32(buffer, (int32_t)(rpm_kp * 1e3), &send_index);
	buffer_append_int32(buffer, (int32_t)(rpm_ki * 1e3), &send_index);
	buffer_append_int32(buffer, (int32_t)(rpm_kd * 1e5), &send_index);

#if defined VESC_CAN1
	CAN_TxMsgEID(&hcan1, controller_id | ((uint32_t)CAN_PACKET_SET_RPM_KP << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan1, controller_id | ((uint32_t)CAN_PACKET_SET_RPM_KI << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan1, controller_id | ((uint32_t)CAN_PACKET_SET_RPM_KD << 8), buffer, send_index / 3);
#elif defined VESC_CAN2
	CAN_TxMsgEID(&hcan2, controller_id | ((uint32_t)CAN_PACKET_SET_RPM_KP << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan2, controller_id | ((uint32_t)CAN_PACKET_SET_RPM_KI << 8), buffer, send_index / 3);
	CAN_TxMsgEID(&hcan2, controller_id | ((uint32_t)CAN_PACKET_SET_RPM_KD << 8), buffer, send_index / 3);
#endif
}
...
//	}
	...
	case CAN_PACKET_POLL_ROTOR_POS:
		...
		break;
	case CAN_PACKET_STATUS_11:
		infolist[vesc_index].rpm_kp = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e3, &indexbuf);
		infolist[vesc_index].rpm_ki = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e3, &indexbuf);
		break;
	case CAN_PACKET_STATUS_12:
		infolist[vesc_index].rpm_kd = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e5, &indexbuf);
		infolist[vesc_index].rpm_kdfilter = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e5, &indexbuf);
		break;
	case CAN_PACKET_STATUS_13:
		infolist[vesc_index].pos_kp = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e3, &indexbuf);
		infolist[vesc_index].pos_ki = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e3, &indexbuf);
		break;
	case CAN_PACKET_STATUS_14:
		infolist[vesc_index].pos_kd = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e5, &indexbuf);
		infolist[vesc_index].pos_kdfilter = buffer_get_float32((uint8_t*)&vescmsg.Data, 1e5, &indexbuf);
		break;
